...
`best_solution_ga_beam.json` and `wheel_step_manifest.json`, which the last real run wrote.

IT READS `best_solution_ga_beam.json`, NOT `best_solution.json`, AND THAT DECOUPLING IS
THE POINT.  This file's job is regression detection on `evaluate_design`, which has
nothing to do with which genome the project ships.  While the two were the same file, any
promotion of a new shipped genome forced a choice between breaking these tests and
RE-BASELINING them onto values regenerated by the very function they exist to check —
and a golden test that re-derives its own expectations detects nothing.  The GA/beam
reference is therefore pinned under its own name and never moves; `best_solution.json` is
free to become whatever the current answer is.

The two files are byte-identical in every block today.  That is the starting condition,
not an invariant: the moment a Stage-3 genome is promoted they diverge, and this file must
keep following the pinned one.
"""
...
def record():
    with open(os.path.join(HERE, "best_solution_ga_beam.json")) as fh:
        return json.load(fh)
...
def test_the_artifact_was_produced_by_these_constants(record):
    ...
    assert geom["rim_radius_mm"] == pytest.approx(W.RIM_RADIUS_MM), (
        f"best_solution_ga_beam.json was produced with RIM_RADIUS_MM="
