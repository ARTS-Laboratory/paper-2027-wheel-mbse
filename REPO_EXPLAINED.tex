...
% Compiles with pdflatex, but it needs THREE passes and not two.  The table of
% contents is long enough that typesetting it shifts the body by a page, so pass 2
% still carries pass 1's page numbers; pass 3 is the first fixed point.  Measured
% 2026-09-08: passes 3, 4, 5 and 6 agree, and pass 2 differs from them in 35 lines
% of the contents.  The committed PDF is the converged one -- a two-pass rebuild
% will show a spurious 35-line diff against it and is NOT a real change.
%     pdflatex REPO_EXPLAINED.tex && pdflatex REPO_EXPLAINED.tex \
%       && pdflatex REPO_EXPLAINED.tex
% or simply:  latexmk -pdf REPO_EXPLAINED.tex        (runs pdflatex 3 times)
...
% A file-and-line citation, e.g. \at{wheel\_wheel.py}{2282}
...
being too soft (\at{wheel\_fea.py}{748--752}, \at{wheel\_objective.py}{1250--1253}).
...
\code{PLAN.md} is about 1.4\,MB and is the project record, organised into numbered
sections (\S1\,\ldots\,\S139 as of 2026-09-08; this paragraph read 778\,KB and \S92 when
it was written, against a file of 773\,KB). Ten sibling \code{*\_PLAN.md} files sit
beside it and eight of them are open work arcs---that second figure has not moved since
\S92, because \code{MBSE\_PLAN.md}
opened as \code{HUBSHARE\_PLAN.md} closed. Comments across the tree cite these by
number. Six closed arc files were deleted on 2026-08-16 and about seventy citations to
them remain dangling on purpose---\at{PLAN.md}{73--107} is the redirect table.
...
place, \code{wheel\_objective.objective} (\at{wheel\_objective.py}{1589--1592}).
...
(\at{wheel\_fea.py}{579}; the \code{jnp} version at \at{wheel\_objective.py}{419}):
%
\begin{equation}
...
weight table (\at{wheel\_objective.py}{399--412}):
...
fix is \code{wheel\_stage3.selection\_key} (\code{:221}), a three-tier lexicographic
...
\end{equation}
%
\at{studies/study\_corner\_singularity.py}{133} solves this by bisection and verifies it
...
\paragraph{(c) The peak was modelled---until \S103, when the mesh grew the fillet.}
For most of this tree's life the constraint was
$K_t(R, t)\,\sigma_{\text{nominal}}(p{=}4) \le \sigma_{\text{allow}}$, with
$K_t = 1 + C(t/2R)^{0.65}$, the Peterson factor from \S\ref{sec:castigliano},
differentiated rather than frozen: an analytic peak standing in for a fillet the
\emph{unfilleted} mesh did not model. \textbf{That constraint was retired on 2026-09-03
and this section went on calling it the shipped one for the five days since.} \S93 wired
\code{fillet=True} in, and on a filleted mesh the surrogate and the thing it stands in
for are both present at once. \S94 measured that double count at exactly $0.0$ at the
shipped genome---utilisation sat below the margin knee, where \code{stress\_margin}
carried the gradient and the wall did not---but \S101 found the disagreement
\textbf{widens} under refinement rather than closing, so being free once did not earn the
surrogate a place.

\S102 built the replacement and \S103 wired it. The shipped constraint
(\at{wheel\_objective.py}{1345--1349}) is now, for each junction
$j \in \{\text{hub}, \text{rim}\}$,
%
\begin{equation}
  \mathrm{util}_j \;=\;
  \frac{\sigma^{\text{region}}_j}{\sigma_{\text{allow}}} \;\;\le\;\; 1,
  \qquad
  \sigma_{\text{allow}} \;=\; \code{ALLOWABLE\_STRESS\_MPA} \;=\; 25.0\ \mathrm{MPa},
  \label{eq:stressconstraint}
\end{equation}
%
where $\sigma^{\text{region}}_j$ is a smooth $p$-norm of von Mises stress in a
bump-kernel tube of radius \code{FILLET\_REGION\_R\_SUP\_MM} $= 0.45$\,mm around
\emph{that junction's own fillet arc}, $\ell_p$-normed at \code{FILLET\_REGION\_P} $= 16$
over the arc's twelve rotational copies (\at{wheel\_adjoint.py}{511}), and then
aggregated across the eight phase samples at \code{stress\_phase\_p} $= 8$. It is still
one \code{soft\_barrier} per junction, summed---the hub priced on
$(\code{R\_hub}, t_0)$, the rim on $(\code{R\_rim}, t_3)$---and still two barriers rather
than one aggregate, because a hard \code{max} over the two would zero the gradient of
whichever is not currently worst and reintroduce exactly the argmax kink the $p$-norm
exists to blur. \textbf{So the peak is measured again, and the thing that changed is that
the mesh now has the feature it is measured on.}

$K_t$ did not leave the file, only the loss. \code{junction\_kt} has exactly one caller in
\code{src/} (\at{wheel\_objective.py}{1288}), and it, \code{hub\_fillet\_cap\_mm} and
\code{hub\_fillet\_r\_effective} now answer a purely geometric feasibility question---does
the requested fillet radius fit the sector the spoke leaves it---that
\code{wheel\_stage3}'s \code{REPORT\_KEYS} and \code{selection\_key} still read. Nothing
they compute reaches \code{util}$_j$.

Note the consequence, and note that it is now \textbf{four} exponents rather than two.
\code{FILLET\_REGION\_P = 16.0} (\at{wheel\_adjoint.py}{436}) is the constraint's, over
the twelve rotational copies; \code{stress\_phase\_p = 8.0}
(\at{wheel\_objective.py}{1117}) aggregates the eight phase samples;
\code{STRESS\_NOMINAL\_P = 4.0} (\at{wheel\_objective.py}{147}) is
\textbf{no longer the constraint's exponent}---since \S103 it is the volume-weighted
Gauss-point $p$-norm behind the reported \code{stress\_utilisation} and
\code{pnorm\_by\_p} diagnostics only, and it is mesh-convergent there (GCI 0.45\% /
0.20\%, observed order 1.76 / 2.18, comparable to the axle drop's 2.44 / 0.14\%); and
\code{wheel\_adjoint.STRESS\_PNORM\_P = 30.0} (\code{:344}) stays the adjoint's documented
default so historical records keep their meaning. Four exponents, four meanings, and the
one this paragraph used to call ``the constraint's'' is the one that is not.
...
\end{equation}
%
by fixed-point iteration from $q = 0$ (\at{studies/study\_deflection\_gci.py}{129}). The
\emph{Richardson extrapolation} to $h \to 0$ and the \emph{Grid Convergence Index} (a
conservative uncertainty band on the extrapolated value) follow at \code{:167}. The
...
\at{wheel\_adjoint.py}{738}, uses \code{scipy.sparse.linalg.factorized} and reuses the
...
(\code{service\_qoi\_value\_and\_grad}, \at{wheel\_adjoint.py}{867}):
%
\begin{align}
  \frac{d\delta^*}{dp}
...
(\code{BLOCK\_ORDER}, \at{wheel\_wheel.py}{2467}): \code{spoke},
...
(\code{\_UnionFind}, \code{:2529}, with path compression and lowest-id-wins so ownership
...
\code{\_orient\_elements} (\code{:3049}) flips whole blocks rather than trusting each
one.
...
\code{phase\_stencil} (\at{wheel\_objective.py}{995}) offers three schemes. The default
...
quantization was a performance fact, retired since \S162 (below).
...
\item \emph{Quantizing} mattered because \code{wheel\_wheel.coord\_fn} keyed its JIT
  cache on \code{float(phase)}: a \emph{continuously} random offset missed on every
  phase of every step and paid a measured 0.774\,s re-trace eight times per step. Since
  \S162 successor 1 the phase is a traced argument, one cache entry serves every phase,
  and \code{\_COORD\_FN\_CACHE\_MAX = 128} is headroom for recipes, not phases. The
  $8\times8 = 64$-point lattice stays because it is what the recorded runs were drawn on.
\end{itemize}
...
(\code{wheel\_stage3.adam\_update}, \code{:274}; \code{project}, \code{:288}).
...
cliff &
  ...
  \code{\_layer\_cliff\_from\_scalars}, \code{:1547} \\
\addlinespace
sector-fit clamp &
...
src/
  ...
  wheel_requirements.py 1091     MISSION -> CONSTANTS, POINTS -> WEIGHTS.  Derives
                                  force, stroke, allowable, modulus, wall floor and the
                                  14-key weight table that `wheel_fea`'s two bare literals
                                  used to stand for.  numpy + `wheel_fea` only.
  wheel_wheel.py     3869        THE FULL 360 DEGREE MESH.  Ring geometry, block builders,
  ...
  wheel_adjoint.py    981        THE GRADIENT.  QoIs, the adjoint solve, load-controlled
                                  total derivatives, LOBPCG buckling eigenpair.
  wheel_objective.py 1683        THE STAGE-3 SCALAR.  Fourteen terms in three tiers, the Kt
                                  stress constraint, the hub fillet cap model, phase
                                  stencils, jit caches, loss breakdown reporting.
  wheel_stage3.py    1317        THE DRIVER.  Projected Adam (and an L-BFGS-B alternative),
                                  step-reject policy, selection key, run records, CLI.
  wheel_pool.py       435        Parent half of the phase worker pool.  stdlib+numpy, NO jax.
  wheel_pool_worker.py 111       Child.  Pins threads BEFORE its first import.
  wheel_mbse.py       258        THE MBSE FRONT END.  A mission and 100 priority points
                                  in; a requirement set, a compliance table and optionally
                                  a warm-started descent out.  `make mbse`.
  wheel_step_export.py 1248      THE EXPORTER (CadQuery env).  Profile construction,
                                  ...
                                  STEP write, manifest.
studies/              38 drivers + their committed .json/.jpg artifacts
tests/                34 files, 632 test functions
\end{lstlisting}

Those counts were measured at \S139 (2026-09-08) and they are a claim about their
instrument as much as about the tree. Lines are \code{wc -l}; drivers are
\code{ls studies/study\_*.py}; test functions are
\code{grep -c '\textasciicircum{}def test\_' tests/*.py}, which counts \emph{definitions}
and not collected cases, so a parametrised test counts once. The same three instruments
read 26 / 503 / 27 at \S92, when this section was first written---and the figures written
here then were 25 / 503 / \textasciitilde{}19, so the file count was already one low and
the driver count last matched the tree on 2026-08-19, ten days before that PDF was built.
\code{wheel\_requirements} and \code{wheel\_mbse} did not exist yet; both landed on
2026-08-31, the day after. Re-derive these; do not cite them.
...
\begin{lstlisting}
      ...
      wheel_geometry --+-- wheel_fea ------> wheel_step_export   [CAD ENV]
       (xp-generic)         (numpy)              (cadquery/OCP)
            |                 |
        wheel_mesh            +-- wheel_requirements   (numpy)
            |                 |
            +---> wheel_wheel + jax_config
                ...
                wheel_objective
                 ...
                 wheel_stage3 ---- wheel_pool ---- wheel_pool_worker
                       |
                  wheel_mbse   (CLI front end; also imports wheel_requirements)
\end{lstlisting}
...
\paragraph{\code{wheel\_wheel.\_COORD\_FN\_CACHE}} (\code{:2838}), the jitted
...
\paragraph{\code{wheel\_objective.\_T1\_CACHE}} (\code{:901}) and
\textbf{\code{\_KT\_CACHE}} (\code{:526}) follow the same pattern for the T1 term vector
...
\code{main()} (\at{wheel\_stage3.py}{1079}) loads \code{best\_solution.json} via
\code{load\_genes} (\code{:980}), converts the 14-key dictionary to an ordered vector
...
\code{descend} (\code{:479}) computes the \textbf{flank orientation} once, from the
starting genes:
...
\code{Evaluator.\_\_call\_\_} (\code{:446}) calls
...
Inside \code{build\_wheel} (\at{wheel\_wheel.py}{2947}):

\begin{enumerate}
\item \code{\_sector\_coords} (\code{:2656}) calls \code{sector\_blocks} (\code{:2282}),
...
\code{WO.objective(z, cfg, normalized=True, ...)} (\at{wheel\_objective.py}{1433})
denormalises, then runs three tiers.

\paragraph{T1} (\code{t1\_vector}, \code{:803})---genes only, no mesh, microseconds via
...
\paragraph{T2} (\code{t2\_vector}, \code{:960})---mesh-space, via \code{mesh\_coords}
alone:
...
\paragraph{T3} (\code{t3\_terms}, \code{:1115})---one
...
(\at{wheel\_stage3.py}{145--169}). It was 1.0, with the rule ``never step further into
...
tracked. The run record (\code{\_record}, \code{:793}) is written incrementally, so a
...
\code{wheel\_wheel.mesh\_coords(genes, mesh)} (\code{:2706}) returns the mesh's node
...
\code{smooth\_min} (\at{wheel\_objective.py}{429}) is worth a close read. The blend
...
limits} (\at{wheel\_objective.py}{165--300}, \code{:733}), and the structure of that
...
(\at{wheel\_adjoint.py}{966}) measures it on the Jacobian with $\mathrm{tol} = 0.0$
